...
- `Webhooks/IWebhooksClient.cs`/`WebhooksClient.cs` — CRUD via API, `CreateAsync` retorna `CreateWebhookResult` (novo tipo — não existia no assessment porque a API não devolvia nada relevante; agora carrega `SigningSecret`, exibido uma única vez, refletindo o `chaveAssinatura` do backend#1445). `ListAsync() : Task<IReadOnlyList<Webhook>>` — **sem paginação**: `GET /v1/webhooks` passou a devolver um array direto (backend#1449/docs#25), porque um parceiro tem no máximo 3 webhooks (um por tipo). Não existe `PagedResult<Webhook>` nem `ListAllAsync` aqui — seria paginar uma lista que nunca passa de 3 itens.
...
**Achado — resolvido no próprio SDK:** com credenciais de teste inválidas, a chamada de geração de token retornava `416 Requested Range Not Satisfiable` do `awselb/2.0` em vez do `400` real. Causa raiz: `ParceleMaisOptions.ResolveBaseUrl()` retornava a URL **sem `/` final** (`https://.../integration`); `HttpClient.BaseAddress` + caminho relativo segue resolução de URI relativa (RFC 3986 §5.3) — sem `/` final, o último segmento do path é **substituído**, não concatenado. Toda chamada real do SDK acabava em `https://.../v1/...`, sem `/integration`, batendo em algo atrás do mesmo domínio que devolve `416`. Não era específico do fluxo de auth nem de TFM — afetava qualquer chamada do SDK. `ResolveBaseUrl()` agora garante `/` final sempre.
...
| `416 Requested Range Not Satisfiable` do ALB em vez do erro real da API | **Resolvido** (branch `fix/http-client-baseaddress-trailing-slash`) | Causa raiz: `ResolveBaseUrl()` sem `/` final — `HttpClient.BaseAddress` + caminho relativo descartava `/integration` (RFC 3986 §5.3). Confirmado comparando o `RequestUri` real enviado pelo `HttpClient` contra uma chamada `curl` equivalente. |
