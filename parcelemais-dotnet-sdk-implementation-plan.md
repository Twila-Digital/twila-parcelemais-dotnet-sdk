...
- `DependencyInjection/ServiceCollectionExtensions.cs` (`AddParceleMais`), `DependencyInjection/ParceleMaisClientBuilder.cs`.
- `ParceleMaisClient.cs` — implementa `IParceleMaisClient`, ainda com `Orders`/`Customers`/`Simulations`/`Webhooks` retornando `null!`/lançando `NotImplementedException` explícito (documentado como "implementado na Fase 3/4", nunca silenciosamente quebrado).

**Confirmação técnica pendente (bloqueia o início desta fase, não da Fase 1):** validar nesta fase, antes de escrever o resto, se `Microsoft.Extensions.Http.Resilience` na versão atual suporta `netstandard2.0`. Se não suportar, cai para o fallback `Polly.Core` direto (assessment §8) — a API pública de `ParceleMaisResilienceOptions` não muda, só a implementação interna de `ResiliencePipelineFactory`.
